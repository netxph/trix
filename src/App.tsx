...
function DartPicker({ onPick }: { onPick: (dart: Dart) => void }) {
  ...
  return <div>
    ...
    <div className="mt-2 grid grid-cols-3 gap-2"><button className={`${button} min-h-16 border-2 border-amber-300 bg-amber-50 text-3xl font-bold tracking-tight text-amber-900`} onClick={() => onPick(bullDart(false))}>25</button><button className={`${button} min-h-16 border-2 border-amber-300 bg-amber-100 text-3xl font-bold tracking-tight text-amber-950`} onClick={() => onPick(bullDart(true))}>50</button><button className={`${button} min-h-16 border-2 border-slate-300 bg-slate-100 text-lg font-bold tracking-tight text-slate-800`} onClick={() => onPick(miss())}>Miss</button></div>
  </div>
}
