...
const button = 'rounded-xl border border-slate-200 px-3 py-3 text-center shadow-sm active:scale-95 disabled:opacity-40'
...
function DartPicker({ onPick }: { onPick: (dart: Dart) => void }) {
  ...
  return <div>
    ...
    <div className="mt-2 grid grid-cols-3 gap-2"><button className={`${button} min-h-16 border-2 border-amber-300 bg-green-100 text-3xl font-bold tracking-tight text-amber-900`} onClick={() => onPick(bullDart(false))}>25</button><button className={`${button} min-h-16 border-2 border-amber-300 bg-red-100 text-3xl font-bold tracking-tight text-amber-950`} onClick={() => onPick(bullDart(true))}>50</button><button className={`${button} min-h-16 border-2 border-slate-300 bg-slate-100 text-lg font-bold tracking-tight text-slate-800`} onClick={() => onPick(miss())}>Miss</button></div>
  </div>
}
...
function Play({ run, onChange, onEnd }: { run: Run; onChange: (run: Run) => void; onEnd: () => void }) {
  ...
  return <main className="mx-auto flex h-dvh max-w-md flex-col overflow-y-auto px-4 pb-3 pt-4">
    ...
    <section className="mt-3 flex min-h-16 flex-1 flex-col overflow-hidden rounded-2xl bg-slate-200/60 p-2">
    ...
    <section className="mt-3 shrink-0"><div className="mb-2 flex items-center justify-between"><h2 className="font-bold">Your three darts</h2><span className="rounded-full bg-sky-100 px-3 py-1 text-sm font-bold text-sky-700">Live score {liveScore}</span></div><div className="grid grid-cols-3 gap-2">{draft.map((dart, index) => <button key={index} aria-label={`Dart ${index + 1} ${dartLabel(dart)}`} className={`relative flex min-h-16 items-center rounded-2xl border-2 p-2 text-left ${selected === index ? 'border-sky-500 bg-sky-50' : 'border-transparent bg-white'}`} onClick={() => setSelected(index)}><span className="absolute right-2 top-2 text-xs font-bold text-slate-400">{dartLabel(dart)}</span><b className="text-3xl font-black">{dart.kind === 'miss' ? 0 : dart.score}</b></button>)}</div></section>
...
function Result({ run, onNext, onNew }: { run: Run; onNext: () => void; onNew: () => void }) {
  ...
  return <main className="mx-auto flex h-dvh max-w-md flex-col overflow-y-auto px-4 py-4">
    {won ? <div className="flex items-center gap-3"><div className="flex-1 rounded-2xl bg-green-600 p-3 text-white"><h1 className="text-3xl font-black">Congratulations!</h1><p className="mt-1 text-white/80">You checked out!</p></div><img className="h-20 w-20 shrink-0" src="./owl.svg" alt="Owl congratulating you" /></div> : <div className="flex items-center gap-3"><div className="flex-1 rounded-2xl bg-red-600 p-3 text-white"><h1 className="whitespace-nowrap text-2xl font-black">Sorry. Try again?</h1><p className="mt-1 text-sm text-white/80">You’ve got this!</p></div><img className="h-20 w-20 shrink-0" src="./owl.svg" alt="Owl comforting you" /></div>}
